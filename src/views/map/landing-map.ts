...
export async function initLandingMap(container: HTMLElement) {
  await loadAmCharts();
  await waitForSize(container);   // ensure amCharts reads a real size
  ...
  const bootAnimation = () => {
    ...
    heroImg.classList.remove('is-map-bound');
    heroImg.classList.remove('facing-right');
    ...
    if (boundsSeed.length) {
      const b = {
        left: Math.min(...boundsSeed.map((bounds) => bounds.left)),
        right: Math.max(...boundsSeed.map((bounds) => bounds.right)),
        top: Math.max(...boundsSeed.map((bounds) => bounds.top)),
        bottom: Math.min(...boundsSeed.map((bounds) => bounds.bottom)),
      };
    ...
    const lightCountry = (iso: string, duration = 520) => {
      ...
      poly.animate({
        key: 'fillOpacity',
        from: poly.get('fillOpacity') ?? 0.05,
        to: 1,
        duration,
    ...
    setTimeout(() => {
      ...
      let stepIndex = 0;
      const moveNext = () => {
        const stop = routeStops[stepIndex];
        if (!stop) return;
        ...
        stepIndex += 1;
        window.setTimeout(moveNext, STEP_DELAY);
      };

      window.setTimeout(moveNext, 260);
    }, STAGE_WALK_MS + SHRINK_MS + 80);
  };
